...
class BreakoutGame {
  ...
  initGame() {
    ...
    this.gameState.ball = { x: 400, y: 530, vx: 4, vy: -4, radius: 8 };
    this.generateBricks();
  }
  ...
  update(deltaTime) {
    ...
    if (ball.y > this.canvas.height) {
      ...
      else { ball.x = 400; ball.y = 530; ball.vx = 4; ball.vy = -4; }
    }
  }
  ...
  render() {
    ...
    this.ctx.fillText('Score: ' + this.gameState.score + ' | Lives: ' + this.gameState.lives + ' | Level: ' + this.gameState.level, 20, 30);
    this.ctx.fillStyle = '#ffd93d';
    this.ctx.fillText('BREAKOUT', this.canvas.width / 2, 20);

...
class PoolBilliardsGame {
  ...
  setupBalls() {
    ...
    const colors = ['#e74c3c', '#f39c12', '#2ecc71', '#3498db', '#9b59b6', '#e67e22', '#1abc9c'];
    ...
    const checkPocket = (ball) => {
      ...
      this.gameState.pockets.forEach(pocket => {
        ...
        if (dist < this.config.pocketRadius) {
          ...
          if (ball === this.gameState.cueBall) {
            ball.x = 200;
            ball.y = 300;
            ball.pocketed = false;
          } else {
  ...
  allBallsStopped() {
    if (Math.abs(this.gameState.cueBall.vx) > 0.1 || Math.abs(this.gameState.cueBall.vy) > 0.1) {
      return false;
    }
    
    return this.gameState.balls.every(ball => 
  ...
  handleInput(input, playerName) {
    ...
    if (input.left) {
      this.gameState.cueAngle -= 2;
    }
    if (input.right) {
      this.gameState.cueAngle += 2;
    }
    if (input.up) {
      this.gameState.cuePower = Math.min(100, this.gameState.cuePower + 2);
    }
    if (input.down) {
      this.gameState.cuePower = Math.max(20, this.gameState.cuePower - 2);
    }
    if (input.action) {
      this.shoot();
    }
  ...
  drawCue() {
    ...
    const cueBall = this.gameState.cueBall;
    const angleRad = this.cueAngle * Math.PI / 180;
    
    const cueLength = 150 + this.gameState.cuePower;
    const startX = cueBall.x - Math.cos(angleRad) * 30;
    const startY = cueBall.y - Math.sin(angleRad) * 30;
    ...
    this.ctx.beginPath();
    this.ctx.moveTo(startX, startY);
    this.ctx.lineTo(startX - Math.cos(angleRad) * 20, startY - Math.sin(angleRad) * 20);
    this.ctx.stroke();
  }
  ...
  drawUI() {
    ...
    this.ctx.textAlign = 'center';
    this.ctx.fillText('POOL', this.canvas.width / 2, 30);
...
window.PoolBilliardsGame = PoolBilliardsGame;